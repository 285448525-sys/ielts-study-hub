...
function renderHeatmap(){
  ...
  $('#heatmap').innerHTML = '<div class="cal-heatmap">' + days.map(d => {
    ...
    const bg = d.sec ? `rgba(79,70,229,${0.15 + alpha*0.85})` : '';
    const col = d.sec ? '#fff' : '';
  ...
  const scale = [
    { c: 'var(--line)', t: '无（没学）' },
    { c: 'rgba(79,70,229,.3)', t: '较少（<1h）' },
    { c: 'rgba(79,70,229,.55)', t: '中等（1-2h）' },
    { c: 'rgba(79,70,229,.8)', t: '较多（2-4h）' },
    { c: 'rgba(79,70,229,1)', t: '很多（≥4h）' },
  ];
