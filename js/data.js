...
function hubLoad(){
  ...
  try{
    ...
    (function migrateWritingTemplates(){
      const titleUpdates = {
        wt_a: '小作文A 动态图（线/柱带年份）',
        wt_b: '小作文B 静态图（饼/表/静态柱）',
        wt_c: '小作文C 地图题（改造前后对比）',
        wt_d: '小作文D 流程图（自然过程/工序/循环）',
        wt_ba: '大作文A 观点型（第一优先背）',
        wt_bb: '大作文B 讨论型（第二优先背）'
      };
      if(Array.isArray(DATA.writing)){
        DATA.writing.forEach(t => { if(t && titleUpdates[t.id]) t.title = titleUpdates[t.id]; });
