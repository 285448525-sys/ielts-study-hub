...
async function aiPlanItem(){
  ...
  const sys = '你是雅思备考日计划教练。考生会写一句话描述今天想完成的目标，常使用她自己的缩写习惯（L=听力篇数、R=阅读篇数）。请读懂考生真实意图，仅对用户明确提到的目标进行拆分与排序，生成今日任务清单。\n'
    ...
    + '- 模考 = 完整限时模考（默认听力模考+阅读模考）；写作 = 写作练习；口语 = 口语练习（按用户写的时长，如"口语1h"）；背词/背单词 = 背单词（默认 30 分钟）。这些按用户原话保留，不擅自改动作。\n'
    + '\n'
    + '【任务要求】\n'
...
async function aiWeekPlan(){
  ...
  const sys = '你是资深雅思备考计划教练。考生会在文本框里写下"接下来几天内要做的一些任务 + 具体安排"，可能是自由格式（含"今天/明天/周X/上午/下午/具体日期"等时间词），也可能只是一份任务清单，且常使用她自己的缩写习惯。请读懂考生的真实意图，把这些任务智能分配到合适的日期。\n'
    ...
    + '- 口语 = 口语练习，考生常写"口语 30min/1h"。按 30 分钟–1 小时记作 1 个任务，名称如"口语 30 分钟（DeepSeek 对话/跟读）"。口语题库必须按天拆分，一天只过一部分。\n'
    ...
    + '5. 每个任务只写名称，不要写预估时长、不要括号说明；任务名称用平白中文（如"听力 第1篇""阅读 第2篇""模考 听力+阅读"），不要出现 S1/S2、S3/S4、P1/P2/P3 等雅思段落代号；模考/刷题后必须紧跟对应复盘（错题复盘 / 精听 / 精读）；L 篇之间、R 篇之间同类分散避免疲劳；最难的 task 排最前。\n'
