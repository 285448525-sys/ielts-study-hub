...
async function aiPlanItem(){
  ...
  const sys = '你是雅思备考日计划教练。考生会写一句话描述今天想完成的目标，常使用她自己的缩写习惯（L=听力篇数、R=阅读篇数）。请读懂考生真实意图，仅对用户明确提到的目标进行拆分与排序，生成今日任务清单。\n'
    ...
    + '- 口N / 口语N（N=数字）= 今天练 N 道口语题，按"P1 第1刷 → P2 第1刷 → P1 第2刷 → P2 第2刷…"轮流展开成 N 个独立任务，命名"口语 P1 第1刷""口语 P2 第1刷""口语 P1 第2刷"…；她写明各 Part 题数（如"口1 P1 口2 P2"）则严格按写明的分布展开。\n'
    + '- 模考 = 完整限时模考（默认听力模考+阅读模考）；写作 = 写作练习；"口语1h"这类只写时长的口语 = 1 个任务按原话保留；背词/背单词 = 背单词（默认 30 分钟）。这些按用户原话保留，不擅自改动作。\n'
    + '\n'
    + '【任务要求】\n'
    + '① 只写任务本身，不要带时间段、不要写预估时长、不要用括号加说明；\n'
    + '② 严格保留用户原句的科目与动作（"过一遍"不改"背诵默写"，"复习"不改"背诵"），不增加用户没要求的动作；\n'
    + '③ 只生成用户明确提到的学习任务，禁止基于弱项自行添加"阅读模考""听力模考"等未提及任务；弱项仅用于排序；\n'
    + '④ 弱项科目多排、优先排；同类任务分散；最难排最前；\n'
    + '⑤ 模考/套题必须整体出现（"听力限时模考""阅读限时模考"），不能拆成"听力 S1""阅读 P1"；\n'
    + '⑥ 输入含顿号/逗号/"各"并列的多个独立单元，必须拆成独立任务并保留原名称（如"观点型、讨论型模板各默写一遍"→"观点型模板默写一遍""讨论型模板默写一遍"）；\n'
    + '⑦ L/R 缩写必须按上面展开成"第N篇"平白编号任务，不得原样保留"L4""R3"这种缩写。\n'
    + '⑧ 口语题数缩写（口N/口语N）同样必须展开成独立任务，不得原样保留"口3"；命名里的 P1/P2 仅限口语 Part 代号（如"口语 P1 第1刷"），听力的 S1/S2、阅读的 P1/P2 仍然禁止。\n'
    + '\n'
...
async function aiWeekPlan(){
  ...
  const sys = '你是资深雅思备考计划教练。考生会在文本框里写下"接下来几天内要做的一些任务 + 具体安排"，可能是自由格式（含"今天/明天/周X/上午/下午/具体日期"等时间词），也可能只是一份任务清单，且常使用她自己的缩写习惯。请读懂考生的真实意图，把这些任务智能分配到合适的日期。\n'
    + '\n'
    + '【考生的缩写与含义（必须先理解再分配）】\n'
    + '- L = 听力（Listening）。"L4" = 做 4 篇听力练习；"L" 后数字 = 篇数。每篇约 15 分钟（做题 5–6 分钟 + 重听错题复盘约等量）。展开成独立任务时直接按"听力 第1篇 / 第2篇…"平白编号，不要写 S1/S2、S3/S4 这类段落代号。\n'
    ...
    + '- 口语 = 口语练习。"口N / 口语N（N=数字）"= N 道口语题（每题约 15 分钟），展开成"口语 P1 第1刷""口语 P2 第1刷""口语 P1 第2刷"…（按 P1→P2 轮流编号，N 个独立任务，可分摊到不同天）；她写明各 Part 分布则严格照写。只写时长的（"口语 30min/1h"）仍记作 1 个任务，名称如"口语 30 分钟"。口语题库必须按天拆分，一天只过一部分。\n'
    ...
    + '5. 每个任务只写名称，不要写预估时长、不要括号说明；任务名称用平白中文（如"听力 第1篇""阅读 第2篇""模考 听力+阅读"），不要出现 S1/S2、S3/S4 这类代号，听力/阅读也不得写 P1/P2/P3（仅口语 Part 代号 P1/P2 允许，出现在"口语 P1 第N刷"命名里）；模考/刷题后必须紧跟对应复盘（错题复盘 / 精听 / 精读）；L 篇之间、R 篇之间同类分散避免疲劳；最难的 task 排最前。\n'
