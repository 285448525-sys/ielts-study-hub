...
ready(async () => {
  ...
  pdEnsureProgress();
  /* design/07 场景闯关 v2 / design/16 句型页 上线开关：任一为 true 时本文件（pdLegacy 兜底引擎）
     让位、不接管「练习」tab（场景 v2 由 scene-drill.js、句型页由 sentence-drill.js 接管）。
     注：开关判断必须在 ready 内（此时接管方脚本已执行完），不能放脚本顶层。 */
  if(window.__SCENE_V2_ON || window.__SENT_V2_ON) return;
