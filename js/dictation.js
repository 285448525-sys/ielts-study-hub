...
5. 模板填空位豁免：标准原句里的 ____ 是模板骨架"填空位"——用户整框留空不填，或只写几个占位下划线，【不算错】，不要判"漏写"，也不要列进 mistakes。只比对用户实际写出的文字与非填空部分是否一致。
6. 标点符号豁免：只比对"文字内容"（单词拼写、词序、有无漏词）。学生默写里多写、少写、或写错一个标点符号（中英文句号、逗号、问号、感叹号、引号等），【不算错】，不要因此判"语法"或"语序"或"漏写"。除非标点差异导致了句子意思或语法结构实质改变（极少见），否则忽略标点。
7. 返回严格 JSON：{"mistakes":[{"loc":"1","wrong":"学生写法(漏写则空字符串)","right":"正确写法","type":"漏写|错词|拼写|语法|语序","note":""}]}，无错则 mistakes:[]。
只输出 JSON，无解释无围栏。` },
    { role:'user', content:
`标准原句：
${correctText}
...
9. 标点符号豁免：只比对"文字内容"（单词拼写、词序、有无漏词）。学生默写里多写、少写、或写错一个标点符号（中英文句号、逗号、问号、感叹号、引号等），【不算错】，不要因此判"语法""语序"或"漏写"。除非标点差异导致句子意思或语法结构实质改变（极少见），否则忽略标点差异。
10. 返回严格 JSON：
