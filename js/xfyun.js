...
/* 构造握手 URL（带鉴权参数）。host/path 可指定：默认走 ISE（发音评测），
   IAT（语音听写）传 'iat-api.xfyun.cn' / '/v2/iat'。 */
async function xfyunAuthUrl(cfg, host, path){
  host = host || 'ise-api.xfyun.cn';
  path = path || '/v2/open-ise';
...
/* =======================================================================
   ...
   与 xfyunEvaluate 同一套握手/音频帧逻辑，差异见内联注释。
   ======================================================================= */
