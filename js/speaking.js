...
var SYS_DIAG =
  ...
  + '评分只关注口语可感知的内容层面（流利度、词汇、语法结构等）；大小写与标点属书写格式层面，一律不计入扣分——例如语音转文字导致的首字母大小写问题、标点被误插到句子中间等，都不影响分数。\n'
  + '2) 指出语法/用词错误：逐条给 原句 → 问题(中文简说，不堆术语) → 修改；没有错误就如实说很少。\n'
  + '注意：不要指出大小写或标点类问题（如首字母大小写、标点位置/缺失、标点误插句中），这些不计错、不纠正；只指出真正的语法/用词内容错误。\n'
  + '3) 在【不改动考生原本思路与想说内容】的前提下，给一版更地道、自然、符合其基础（简单句型、常见词汇）的英文重写（重写只优化内容表达与地道性，不必为大小写/标点做格式性修改）。\n'
...
var SYS_DIAG_P2 =
  ...
  + '评分只关注口语可感知的内容层面（流利度、词汇、语法结构等）；大小写与标点属书写格式层面，一律不计入扣分（如语音转文字导致的首字母大小写、标点误插句中等都不影响分数）。\n'
  + '2) 【语法纠错】逐条指出语法/用词错误：原句 → 问题(中文简说) → 修改；没有就如实说很少。\n'
  + '不要指出大小写或标点类问题（首字母大小写、标点位置/缺失、标点误插句中），这些不计错、不纠正。\n'
