...
async function aiStoryLink(id){
  ...
  try{
    ...
    const sys = [
      ...
      '2. 搬运比例（硬性红线）：全文 **65% 以上的词必须从语料库原句直接搬运**，你新加/改写的内容（含点题句、开头结尾的改写）合计**不得超过全文词数的 35%**。除 1~2 句点题句外，只允许做词级微调（时态/人称/单复数），严禁整句重写语料库原句。严禁编造生僻细节（展览内容、建筑外观、名人成就、菜品味道等）；若题目所涉事物不在语料库，用 "Well, actually, ..." 明说，并硬套素材里的风景/感受类句子，绝不编造新内容。',
      ...
      '8. 黑体标注 = 新增内容：所有不属于语料库原文的新加/改动词句必须用 ** 包裹标黑体（考生靠黑体一眼看出哪些是临场要加的）；黑体部分总词数不得超过全文 35%，语料库原句一律不标。',
